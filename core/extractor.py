...
class MultiBasicEncoder(nn.Module):
    def __init__(self, output_dim=[128], encoder_dim=[64, 96, 128], norm_fn='batch'):
        super(MultiBasicEncoder, self).__init__()
        self.norm_fn = norm_fn

        # output convolution for feature
        self.conv2 = nn.Sequential(
            ResidualBlock(encoder_dim[2], encoder_dim[2], 'instance', stride=1),
        ...
        for dim in output_dim:
            conv_out = nn.Sequential(
                ResidualBlock(encoder_dim[2], encoder_dim[2], self.norm_fn, stride=1),
